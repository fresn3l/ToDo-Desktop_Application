...
class IphoneIntentsTests(unittest.TestCase):
    def test_shortcuts_and_widget_use_the_same_actions(self) -> None:
        ...
        self.assertIn("static func addJournal(", actions)
        self.assertIn("JournalScreen.swift", project)
        self.assertIn("Parked in To Do.", intents)
        self.assertIn("Park in To Do", intents)
        self.assertIn("static func assignDay(", actions)
        self.assertIn("static func markBar(", actions)
        self.assertIn("static func completeClock(", actions)
    ...
    def test_three_tabs_and_pack_actions(self) -> None:
        ...
        self.assertIn("PackActions.assignDay", todo)
        self.assertIn('source != "calendar"', todo)
        self.assertIn('navigationTitle("To Do")', todo)
        self.assertIn("QuietSyncButton", todo)
        self.assertIn("DayPickSheet", todo)
        self.assertIn("QuietSyncButton", today)
        ...
        self.assertIn("This day or the series?", calendar)
        self.assertIn("QuietSyncButton", calendar)
        self.assertIn("case today, calendar, work, journal", app)
        self.assertIn('case .work: return "To Do"', app)
        self.assertIn("JournalScreen", app)
        self.assertNotIn("InboxScreen", app)
        self.assertNotIn("Due soon", todo)
        journal = (ROOT / "ios" / "Kosistenz" / "JournalScreen.swift").read_text(encoding="utf-8")
        self.assertIn("QuietSyncButton", journal)
        self.assertIn("needsFolder", journal)
