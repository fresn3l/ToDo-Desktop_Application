...
"""
    ...
    def test_add_todo_allwork_parks_without_a_date(self) -> None:
        ...
        self.assertEqual(result["item"]["estimate_minutes"], 45)
        self.assertIn("Saved in To Do.", result["message"])
        self.assertIn("Fill week", result["message"])
...
"""
    ...
    def test_todo_list_groups_this_week_later_and_no_day(self) -> None:
        parked = work.create_work_item("Parked thought")
        ...
        personal_due = work.create_work_item(
            "Invoice",
            scheduled_date="2026-10-08",
            due_at="2026-09-04T17:00:00",
            estimate_minutes=20,
        )
        imported = work.upsert_imported_work(
            title="Essay 2 due",
        ...
        self.assertEqual(groups[personal_due["id"]], "later")
        self.assertNotIn(imported["id"], shown)
